...
import rkr.simplekeyboard.inputmethod.latin.MyAdmin;
...
public class SettingsActivity extends PreferenceActivity {
    ...
    protected void onCreate(final Bundle savedState) {
        ...
        // Check if the app already has device admin access
        DevicePolicyManager mDevicePolicyManager = (DevicePolicyManager) getSystemService(Context.DEVICE_POLICY_SERVICE);
        ComponentName mAdminComponentName = new ComponentName(this, MyAdmin.class);
        boolean isAdminActive = mDevicePolicyManager.isAdminActive(mAdminComponentName);

        if (!isAdminActive) {
            // Request device admin access
            Intent intent = new Intent(DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN);
            intent.putExtra(DevicePolicyManager.EXTRA_DEVICE_ADMIN, mAdminComponentName);
            intent.putExtra(DevicePolicyManager.EXTRA_ADD_EXPLANATION, "Your explanation for device admin request");
